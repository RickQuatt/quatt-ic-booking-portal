/**
 * /kennismaking/check-in
 *
 * Public check-in page for kennismaking meetings. Partner scans the QR on the
 * AM's slide / printed card and lands here. Submits email + name + company. On
 * success, attendance is recorded as a kennismaking_completed milestone in
 * Wall-E OS, which writes `ic__kennismaking_completed=true` on the HubSpot
 * deal via the property-drain worker.
 *
 * Mirrors /training/check-in (same form layout + copy structure). Track-aware
 * is N/A for kennismaking (single track today).
 *
...
export function KennismakingCheckInPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/kennismaking/check-in", {
        method: "POST",
    ...
    return (
      <div className="max-w-lg mx-auto px-6 py-16 text-center">
        ...
        <p className="mt-3 text-[#4D4D4A] leading-relaxed">
          Bedankt. Veel plezier bij de kennismaking.
        </p>
      </div>
    );
  }
