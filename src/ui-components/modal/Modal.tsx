...
export function Modal({
  ...
  return ReactDOM.createPortal(
    <DimmerOverlay>
      <ModalInner closeModal={closeModal}>{children}</ModalInner>
    </DimmerOverlay>,
    rootDOMNode,
  );
}
...
export const ModalInner = ({ closeModal, children }: ModalInnerProps) => {
  ...
  return (
    <div ref={modalRef} tabIndex={0} className={classes.modal}>
      {children}
    </div>
  );
};
...
export const ModalActions = ({ children }: { children: React.ReactNode }) => (
  <div className={classes["modal-actions"]}>{children}</div>
);
